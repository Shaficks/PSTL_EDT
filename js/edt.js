function edt(choix) {
    ...
    for (var i = 0; i < choix_poids.length; i++)
        if (EDT.indexOf(listeUE[choix_poids[i][1]][0]) == -1) //Si l'horaire du cours de l'UE est libre(absent) dans EDT on l'ajoute. 
            EDT.push(listeUE[choix_poids[i][1]][0]); //On ajoute ici les cours des autres UEs
    //alert("EDT Cours=" + EDT);

    if (EDT.length == choix_poids.length) //Si la longueur de EDT differente de 6 on stope la generation:Chevauchement detecte(EDT.length<6 => certains horaires n'ont pas pu etre ajoutes (collision)))
